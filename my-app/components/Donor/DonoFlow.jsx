...
//   // Function to submit donation data to backend
...
//     }
//     if (imageMetadata) {
//       formData.append("device", imageMetadata.device);
//       formData.append("dateTime", imageMetadata.dateTime);
//     }
...
//   // Handle file input change and extract metadata
...
//         const dateTimeOriginal = EXIF.getTag(this, "DateTimeOriginal") || "Unknown Date/Time";
//         setImageMetadata({
...
//               <p>
//                 <strong>Device:</strong> {imageMetadata.device}
//               </p>
//               <p>
//                 <strong>Date & Time:</strong> {imageMetadata.dateTime}
//               </p>
//             </div>
//           )}
...
const DonorFlow = () => {
  ...
  // Handle file input change and extract metadata from any uploaded image
  const handleFileChange = (e) => {
    if (e.target.files && e.target.files[0]) {
      const file = e.target.files[0];
      if (file.type.startsWith("image/")) {
  ...
  // Extract metadata (device and date/time when the photo was taken) using EXIF
  const extractImageMetadata = (file) => {
    const objectURL = URL.createObjectURL(file);
    const img = new Image();
    img.src = objectURL;
    ...
    <div className="min-h-screen flex flex-col items-center justify-center bg-gray-100 p-6">
      <div className="bg-white shadow-lg rounded-lg p-8 w-full max-w-md">
        ...
        <form onSubmit={handleDonorFormSubmit}>
          ...
          {imageMetadata && (
            <div className="p-4 mt-2 text-green-500 bg-gray-200 rounded-lg">
